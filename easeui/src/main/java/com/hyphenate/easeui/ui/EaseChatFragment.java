...
import com.hyphenate.easeui.model.RongFileProvider;
...
//                                    false).show();
    ...
    class MyItemClickListener implements EaseChatExtendMenu.EaseChatExtendMenuItemClickListener{
        ...
        public void onClick(int itemId, View view) {
            ...
            case ITEM_LOCATION:
                //用户点击了地图
                new EaseAlertDialog(getContext(),"通知","开发中敬请期待",R.style.dialog,null,
                        false).show();
                //Toast.makeText(getContext(),"开发中敬请期待",Toast.LENGTH_SHORT).show();
    ...
    private void doNext(int requestCode, int[] grantResults) {
        ...
        if (grantResults != null && grantResults.length > 0 && requestCode == 1) {
            if (grantResults[0] == PackageManager.PERMISSION_GRANTED) {
                ...
                // Permission Denied
                new EaseAlertDialog(getContext(),"提示","您没有允许相机权限",R.style.dialog,null,
                        false).show();
            }
        }
...
// 这个是activity中用的当前是在 fragment中不能用这个方法
// ActivityCompat.requestPermissions(getActivity(),
//         new String[]{Manifest.permission.WRITE_EXTERNAL_STORAGE},
//         0);


//这是环信的代码 -  安卓6.0以后要动态获取用户权限这个要自己实现一下 ↓ 有问题
//            cameraFile = new File(PathUtil.getInstance().getImagePath(), EMClient.getInstance().getCurrentUser()
//                    + System.currentTimeMillis() + ".jpg");
